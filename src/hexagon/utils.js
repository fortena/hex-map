...
export const flatToppedGrid = (oX, oY, size, gridX, gridY) => {
  const width = size * 2;
  ...
  return {
    ...
    gridX,
    gridY,
    corners: hexCorners(FLAT, x, y, size),
  };
};

export const gridPoint = (oType, oX, oY, size, gridX, gridY) => {
  if (oType === POINTY) {
    return pointyToppedGrid(oX, oY, size, gridX, gridY);
  } else if (oType === FLAT) {
    return flatToppedGrid(oX, oY, size, gridX, gridY);
  } else {
